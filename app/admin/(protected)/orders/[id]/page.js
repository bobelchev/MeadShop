...
import WaybillButton from '../WaybillButton';
...
export default async function AdminOrderDetailPage({ params }) {
  ...
  return (
    <div className="max-w-2xl">
      ...
      {['ekont_office', 'ekont_door'].includes(order.delivery_method) && (
        <div className="mb-6">
          <h2 className="text-base font-semibold text-gray-800 mb-2">Econt Waybill</h2>
          {order.econt_shipment_number ? (
            <div className="bg-white border border-gray-200 rounded-lg p-4 text-sm space-y-2">
              <p>
                <span className="text-gray-500 w-36 inline-block">Shipment №</span>
                <span className="font-mono font-medium text-gray-800">{order.econt_shipment_number}</span>
              </p>
              {order.econt_waybill_url && (
                <a
                  href={order.econt_waybill_url}
                  target="_blank"
            ...
            <WaybillButton orderId={order.id} />
          )}
        </div>
      )}
