...
export async function sendOrderNotification({ orderId, customerName, phone, email, deliveryMethod, addressOrOffice, city, notes, totalAmount }) {
  ...
  const bodyHtml = `
    ...
    <p style="margin:0;font-size:15px;color:#2A1A0E;">
      Сума: <strong>${(Number(totalAmount) / EUR_TO_BGN).toFixed(2)} EUR</strong> <span style="color:#7A6C5C;font-size:13px;">(${Number(totalAmount).toFixed(2)} лв.)</span>
    </p>
    ${divider()}
    <p style="margin:0;">
...
export async function sendOrderConfirmation({ orderId, customerName, email, deliveryMethod, addressOrOffice, city, notes, items, totalAmount, locale = 'bg' }) {
  ...
  const itemRows = items.map(({ name, qty, unitPrice }) => `
    <tr>
      ...
      <td style="padding:8px 0;font-size:13px;color:#2A1A0E;text-align:right;border-bottom:1px solid #F5EDDC;">${(unitPrice * qty / EUR_TO_BGN).toFixed(2)} EUR <span style="color:#7A6C5C;font-size:11px;">(${(unitPrice * qty).toFixed(2)} лв.)</span></td>
    </tr>`).join('');
  ...
  const bodyHtml = isBg ? `
    ...
    <table width="100%" cellpadding="0" cellspacing="0">
      ...
      <tr>
        ...
        <td style="padding:12px 0 4px;font-size:15px;font-weight:bold;color:#2A1A0E;text-align:right;">${totalEur} EUR <span style="font-size:12px;font-weight:normal;color:#7A6C5C;">(${totalBgn} лв.)</span></td>
      </tr>
    </table>
  ...
  const textItems = items.map(({ name, qty, unitPrice }) => `  - ${name} × ${qty} — ${(unitPrice * qty / EUR_TO_BGN).toFixed(2)} EUR (${(unitPrice * qty).toFixed(2)} лв.)`).join('\n');
  const text = isBg
    ? `Благодарим, ${customerName}!\n\nПоръчка #${orderId}\n\n${textItems}\n\nСума: ${totalEur} EUR (${totalBgn} лв.)\nДоставка: ${deliveryLabel} — наложен платеж\nАдрес: ${addressOrOffice}, ${city}${notes ? `\nБележки: ${notes}` : ''}`
...
export async function sendOrderStatusEmail({ orderId, customerName, email, status, shipmentNumber }) {
  if (!process.env.SMTP_HOST || !email) return;
  if (!['confirmed', 'shipped'].includes(status)) return;

  const isShipped = status === 'shipped';

  const bodyHtml = isShipped ? `
    <p style="margin:0 0 4px;font-size:12px;color:#D4940C;letter-spacing:0.08em;text-transform:uppercase;">Поръчката е изпратена</p>
    <h1 style="margin:0 0 8px;font-size:22px;color:#2A1A0E;">Поръчката Ви е на път!</h1>
    <p style="margin:0 0 24px;font-size:14px;color:#7A6C5C;">Здравейте, ${customerName}! Поръчка #${orderId} е предадена на куриер.</p>
    ${shipmentNumber ? `
    ${divider()}
    <table width="100%" cellpadding="0" cellspacing="0">
      ${labelValue('Номер на пратка', shipmentNumber)}
    </table>` : ''}
    ${divider()}
    <p style="margin:0;font-size:13px;color:#7A6C5C;">При въпроси се свържете с нас на <a href="mailto:${process.env.NOTIFY_EMAIL}" style="color:#D4940C;">${process.env.NOTIFY_EMAIL}</a></p>
  ` : `
    <p style="margin:0 0 4px;font-size:12px;color:#D4940C;letter-spacing:0.08em;text-transform:uppercase;">Поръчката е потвърдена</p>
    <h1 style="margin:0 0 8px;font-size:22px;color:#2A1A0E;">Поръчката Ви е потвърдена!</h1>
    <p style="margin:0 0 24px;font-size:14px;color:#7A6C5C;">Здравейте, ${customerName}! Поръчка #${orderId} е приета и се подготвя за изпращане.</p>
    ${divider()}
    <p style="margin:0;font-size:13px;color:#7A6C5C;">При въпроси се свържете с нас на <a href="mailto:${process.env.NOTIFY_EMAIL}" style="color:#D4940C;">${process.env.NOTIFY_EMAIL}</a></p>
  `;

  const subject = isShipped
    ? `Поръчка #${orderId} е изпратена — Медовината`
    : `Поръчка #${orderId} е потвърдена — Медовината`;

  const text = isShipped
    ? `Здравейте, ${customerName}!\n\nПоръчка #${orderId} е предадена на куриер.${shipmentNumber ? `\nНомер на пратка: ${shipmentNumber}` : ''}`
    : `Здравейте, ${customerName}!\n\nПоръчка #${orderId} е потвърдена и се подготвя за изпращане.`;
  ...
  try {
    ...
    console.error('[email] Failed to send order status email:', err?.message ?? err);
  }
}
